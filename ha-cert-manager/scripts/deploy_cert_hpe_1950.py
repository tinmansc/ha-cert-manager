...
def load_switches(switches_file=None):
    ...
    if not path.exists():
        raise SystemExit(
            ...
            "directly in HA Cert Manager (credentials and switch fields in device editor).\n\n"
            "Example manual file:\n\n"
            "  switches:\n"
            "    LoftPoESwitch:\n"
            ...
            "The switch name must match the device name configured in HA Cert Manager."
        )
...
def load_secrets():
    # Accept credentials from env vars (set by HA Cert Manager's comware.py).
    ...
    if not SECRETS.exists():
        raise DeployError(
            f"Credentials not found. Either configure them in the HA Cert Manager "
