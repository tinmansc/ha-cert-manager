...
### Added
- **HP printer device type** — deploys the local Let's Encrypt cert to an HP printer's EWS as a PKCS#12 import (verified against a real HP DeskJet 2752e). HP gates its certificate API behind the EWS admin password (HTTP Basic) — with no password set the cert endpoints are invisible, which is a genuine gotcha. Deploy bundles leaf+chain+key into a PKCS#12 and POSTs it to `/Security/DeviceCertificates/NewCertWithPassword/Upload`; the uploaded cert becomes the active HTTPS cert immediately (no separate activation or reboot), so a TLS serial probe verifies both `check` and the post-deploy result. **Two real-device quirks were caught and handled by round-trip testing**: the cert API lives under its own `/CertificateManifest.xml` (not the `/DevMgmt/` DiscoveryTree) and is auth-gated; and HP's embedded multipart parser rejects the upload (LEDM 400) unless the `certificate` file part comes **before** the password fields — so the request is built as an ordered parts list, file-first, rather than the usual `files=`+`data=`.
- **WiCAN Pro (OBD2) device type** — a deliberately unusual one: the MeatPi WiCAN Pro has **no HTTPS server** (its config UI is plain HTTP), so there's no server cert to deploy to it. Its only TLS facility is a client-side *Certificate Manager* for its outbound MQTT connection. So this deployer does a **trust-store push**: it uploads the local Let's Encrypt fullchain as the **CA of a named cert set** (`POST /cert_manager/sets?name=…`, multipart), so the WiCAN can trust an MQTT broker that presents that Let's Encrypt certificate; re-pushed on renewal. The device's API only reports whether a set *exists* (not its contents), so `check()` confirms presence and `deploy()` (idempotent overwrite) keeps it fresh — surfaced honestly as a warning. No TLS fingerprint probe (there's no HTTPS to probe), and the cert-coverage check is skipped for this type since the cert isn't for the device's own hostname. **Verified end-to-end against a real WiCAN Pro** (check → deploy → check → no-local-cert), with the probe/test cert set cleaned up afterward.
- **Trailing/leading whitespace handling on input fields** — clipboard copy/paste routinely appends a stray space, which previously flowed straight into a hostname/URL/key and caused cryptic downstream failures (the motivating case: a pasted git URL with a trailing space threw a long inscrutable error elsewhere). Now: non-secret fields (host, username, site/node/jail names, SSH host/user, PKI domain, SSL policy, paths, etc.) are silently trimmed on save; passphrase-style secrets (device password, API key/XTD CLI password/token secret, PKCS#12 password, and the encryption key) are **not** silently mutated — a leading/trailing space is rejected with an inline "can't start or end with a space" message, and nothing about it is logged (even noting that a secret had a stray space would leak information about it). The backend also trims non-secret fields defensively at config-read time, so values arriving by import/restore/hand-edit are covered too. New standing check added to `RELEASE_CHECKLIST.md` §10 to keep this in mind whenever input fields change.
